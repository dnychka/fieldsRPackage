...
good<- !is.na( y)
y<-y[good]
s<- ozone2$lon.lat[good,]
O3Obj<- fastTps( s,y, aRange=1.5 )
set.seed(122) # set the random seed so we get the same results
lonR<- range(s[,1])
latR<- range(s[,2])
gridList<- list( x= seq(lonR[1], lonR[2],length.out=150 ),
                 y= seq(latR[1], latR[2],length.out=150 )
)
O3Sim<- simLocal.spatialProcess( 
   O3Obj,gridList, M=5, verbose=TRUE)
imagePlot( O3Sim$x, O3Sim$y, O3Sim$z[,,2],
           zlim=c(15,150), col=turbo(256)
           )
